Describe "Get-D365CeODataEntityDataByKey Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-D365CeODataEntityDataByKey).ParameterSets.Name | Should -Be 'Default', 'Specific'
		}
		
		It 'Should have the expected parameter EntityName' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['EntityName']
		...
		It 'Should have the expected parameter Key' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['Key']
		...
		It 'Should have the expected parameter ODataQuery' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['ODataQuery']
		...
		It 'Should have the expected parameter Charset' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['Charset']
		...
		It 'Should have the expected parameter Tenant' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['Tenant']
		...
		It 'Should have the expected parameter URL' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['URL']
		...
		It 'Should have the expected parameter ClientId' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['ClientId']
		...
		It 'Should have the expected parameter ClientSecret' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['ClientSecret']
		...
		It 'Should have the expected parameter FullODataMeta' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['FullODataMeta']
		...
		It 'Should have the expected parameter EnableException' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['EnableException']
		...
		It 'Should have the expected parameter OutputAsJson' {
			$parameter = (Get-Command Get-D365CeODataEntityDataByKey).Parameters['OutputAsJson']
	...
	Describe "Testing parameterset Default" {
		<#
		Default -
		Default -ODataQuery -Charset -Tenant -URL -ClientId -ClientSecret -FullODataMeta -EnableException -OutputAsJson
		#>
	}
 	Describe "Testing parameterset Specific" {
		<#
		Specific -EntityName -Key
		Specific -EntityName -Key -ODataQuery -Charset -Tenant -URL -ClientId -ClientSecret -FullODataMeta -EnableException -OutputAsJson
		#>
	}
